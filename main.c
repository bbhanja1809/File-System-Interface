...
void recursive_delete(const char *path)
{
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        ...
        {
            // If the target is a directory, recursively delete it
            recursive_delete(child_path);
        }
        else
        {
            // If the target is a file, delete it directly
    ...
    // After deleting all files and subdirectories, remove the current directory
    if (rmdir(path) != 0)
    {
        perror("rmdir");
    }
